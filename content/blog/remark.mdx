...
One of Convex's standout features of Convex is **real-time updates**.
...
Retries also happen automatically, when a network connection is restored after an offline period, Convex will reconnect immediately and updates with fresh data when possible.
...
to fetch the next 40. Currently "load more" is manual, but in the future, a cursor/position based automatic refetch feature will be implemented, where based on the cursor position or current position in the list will automatically trigger a fetch.
...
{/* prettier-ignore */}
<div className="ml-8">
  - **Indexed queries**: comments when posted are attached indexes (`postId`,`userId`) which enables fast lookups when requested later. 
  - **Optimistic UI patterns**: previous known states (eg: signed-in status) are used as default on load to
